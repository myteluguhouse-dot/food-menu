(function () {
  ...
  function ingredientsHTML(ingredients) {
    ...
    return `<p class="card-ingredients">${ingredients.join(", ")}</p>`;
  }

  function mayContainHTML(mayContain) {
    if (!mayContain || !mayContain.length) return "";
    return `<p class="card-allergens">May contain: ${mayContain.join(", ")}</p>`;
  }
    ...
    return `
      <article class="card" data-category="${item.category}">
        ...
        <div class="card-body">
          ...
          ${mayContainHTML(item.mayContain)}
        </div>
      </article>`;
  }
